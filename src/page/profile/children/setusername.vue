    ...
    <section class="setname">
      <section class="setname-top">
        <input type="text" placeholder="输入用户名" :class="{'setname-input':bordercolor}" @input="inputThing"
               v-model="inputValue">
        <div>
          <p v-if="earn">用户名是存放快递的重要信息（2到30字符之间）</p>
          <p class="unlikep" v-else>用户名长度在2到30字符之间</p>
        </div>
      </section>
      <section class="reset">
...
//    mixins: [getImgPath],
    ...
    methods: {
      ...
      inputThing() {
        if (this.inputValue.length < 2 || this.inputValue.length > 30) {
          this.earn = false;
      ...
      resetName() {
        this.modifyCustomerName();
      },
      async modifyCustomerName() {
        let _self = this;
        let phoneNumber = getStore('phoneNumber');
        let url = api.expressBase + api.customers + api.osPlatformSeparator + phoneNumber + api.osPlatformSeparator + 'modifyCustomerName';
        let data = {
          "name" : _self.inputValue
        }
        var res = http.put(url, data, api.headers);
        res.then(async function (res) {
          _self.RETSET_NAME(_self.inputValue);
          _self.$router.go(-1);
        });
      },
    }

<template>
  <div class="">
    <head-top head-title="寄件查询" goBack="true"></head-top>
...
//      this.getHandleExpressInventory()
    },
    created() {
    ...
    methods: {
      ...
      async getHandleExpressInventory() {
        let _self = this;
        let url = api.expressBase + api.recordSendExpress + api.osPlatformSeparator+ this.shopCode +api.osPlatformSeparator + this.providerId + "/currentMonthData";
        var res = http.get(url, '', api.headers);
        res.then(async function (res) {
//          console.info('currentMonthData:' + JSON.stringify(res.data));

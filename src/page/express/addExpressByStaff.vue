<template>
  <div class="expressFormContainer">
    ...
    <form class="loginForm">
      ...
      <section class="input_container">
        <input type="text" placeholder="快递服务商" v-model="expressProvider" @click="getExpressProvider(expressProvider)">
      </section>
      <section class="input_container">
    ...
    <mu-bottom-sheet :open="providerBottomSheet" @close="closeProviderBottomSheet">
      <mu-list @itemClick="closeProviderBottomSheet" style="height: 255px;overflow: scroll;">
        <mu-sub-header>
          请选择快递公司
        </mu-sub-header>
        <mu-list-item v-for="item in providerOptions" :title="item.NAME" :value="item"/>
      </mu-list>
    </mu-bottom-sheet>
  </div>
</template>
...
//      }
    ...
    methods: {
      closeProviderBottomSheet(line){
        this.providerBottomSheet = false;
        if(line != 'overlay') {
          this.providerId = line.value.ID;
          this.expressProvider =line.value.NAME;
        }
//        console.info(this.providerId+','+this.expressProvider);
      },
      ...
      openProviderBottomSheet () {
        this.providerBottomSheet = true;
      },
      getProviderOptions(){
        let _self = this;
...
//          console.info('providerOptions:'+JSON.stringify(res))
          _self.providerOptions = res.data
        });
      },
      async inExpress() {
        let _self = this;
...
//        alert("扫码结果：" + msg);
      ...
      getExpressProvider(expressProvider){
//        console.info(JSON.stringify(expressProvider))
        this.openProviderBottomSheet();
      },

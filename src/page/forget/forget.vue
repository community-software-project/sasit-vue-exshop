            ...
            async resetButton(){
               let _self = this;
                if (!_self.phoneNumber) {
                  ...
                  _self.alertText = '请输入正确的账号';
                    return
                }else if(!_self.oldPassWord){
                  _self.showAlert = true;
                  _self.alertText = '请输入旧密码';
                    return
                }else if(!_self.newPassWord){
                  _self.showAlert = true;
...
//                let res = await changePassword(this.phoneNumber, this.oldPassWord, this.newPassWord, this.confirmPassWord, this.mobileCode);
              ...
              let oldBase64code = Base64.encode(_self.oldPassWord);
              let oldPd = _self.handleStr(oldBase64code,oldSlat);
              let oldPassword = _self.arrayToStr(oldPd);
              let subUrl = _self.phoneNumber+api.osPlatformSeparator+'initialize'+api.osPlatformSeparator+oldPassword
              let url = api.expressBase+api.customers+api.osPlatformSeparator+subUrl;
